...
#define RADAR_UART_RX_PIN   3       // XIAO ESP32-S3 D2 / GPIO3: STM32 USART2_TX (PA2) buraya bağlanmalı
#define RADAR_UART_TX_PIN   4       // XIAO ESP32-S3 D3 / GPIO4: İsteğe bağlı, STM32 RX tarafına bağlanabilir
...
String buildRadarMetadataJson(const RadarMetadata& radar) {
    ...
    json += "\"final\":";
    json += radar.finalDetected ? "true" : "false";
    json += ",\"confirm_count\":";
    json += String(radar.confirmCount);
...
bool isTrueValue(const String& value) {
...
bool parseRadarMessage(String line, RadarMetadata& radar) {
    ...
    radar.rawLine = line;
    radar.finalDetected = false;
    radar.confirmCount = 0;
    radar.objectClass = "";
    ...
    while (start < line.length()) {
        ...
        if (separator > 0) {
            ...
            if (key == "FINAL") {
                radar.finalDetected = isTrueValue(value);
                hasFinal = true;
            } else if (key == "CONFIRM_COUNT") {
                radar.confirmCount = (uint16_t)value.toInt();
            } else if (key == "OBJ") {
                value.toUpperCase();
    ...
    // ESP32 tarafı sadece STM32'nin kesin doğrulama satırıyla tetiklenecek.
    return hasFinal &&
           radar.finalDetected &&
           hasObjectClass &&
           hasMotion &&
           radar.motionDetected &&
           radar.objectClass == "MOTOR";
}
...
void handleRadarLine(const char* line) {
    ...
    if (!parseRadarMessage(String(line), radar)) {
        // Normal OBJ=... ara karar satırları burada sessizce elenir.
        // İstersen debug için aşağıdaki iki satırı açabilirsin.
        // Serial.print("Yok sayıldı: ");
        // Serial.println(line);
        return;
    }

    Serial.print("FINAL=TRUE MOTOR doğrulaması geldi, çekim tetiklendi: ");
    Serial.println(radar.rawLine);
    Serial.printf("CONFIRM_COUNT=%u, OBJ=%s, MOTION=%d, POWER=%.2f, FREQ_HZ=%.2f, SPEED_KMH=%.2f\n",
                  radar.confirmCount,
                  radar.objectClass.c_str(),
                  radar.motionDetected ? 1 : 0,
                  radar.signalPower,
                  radar.peakFrequencyHz,
                  radar.estimatedSpeedKmh);
...
void setup() {
    ...
    Serial.println("ESP32-S3 UART ayari: STM32 PA2 TX -> ESP32 D2 / GPIO3 RX");
    Serial.println("D7/GPIO44 kullanilmiyor; testte calisan D2/GPIO3 kullaniliyor.");
